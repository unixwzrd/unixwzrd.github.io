...
def fix_frontmatter(file_path: Path) -> bool:
 ...
 # Fix specific fields
 if "date" not in fm_data and any(
 parent.name in ["_posts", "_drafts"] for parent in file_path.parents
 ):
 ...
 if "image" in fm_data and (
 '"' in fm_data["image"] or not fm_data["image"].startswith("/")
 ):
