...
def list_missing_pages():
    ...
    for page, data in missing_data['missing_pages'].items():
        ...
        if days_missing >= 7 or data['failure_count'] >= 10:
            print(f"   ⚠️  CONSIDER REMOVAL: Missing for {days_missing} days or {data['failure_count']} failures")
        print()
...
def check_removal_alerts():
    ...
    try:
        ...
        print(f"python3 utils/bin/manage_monitoring_pages.py remove {' '.join(alert_data['pages_to_remove'])}")
...
def remove_pages(pages_to_remove, confirm=False):
    ...
    if not confirm:
        ...
        print(f"python3 utils/bin/manage_monitoring_pages.py remove {' '.join(pages_to_remove)} --confirm")
        return False
...
def main():
    ...
    parser.add_argument('action', choices=['list', 'missing', 'removed', 'alerts', 'remove'],
    ...
    if args.action == 'list':
        ...
        if not args.pages:
            ...
            print("Usage: python3 utils/bin/manage_monitoring_pages.py remove page1 page2 --confirm")
            sys.exit(1)
