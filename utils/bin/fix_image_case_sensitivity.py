...
class ImageCaseFixer:
                            ...
                            if match.group(1) and match.group(1).startswith('/'):
                                ...
                                if normalized_ref in self.actual_files:
                                    ...
                                    if normalized_ref != actual_ref:
                                        file_refs.append((match.start(), match.group(1), actual_ref))
    ...
    def verify_references(self, references: Dict[str, List[Tuple[int, str, str]]]) -> None:
                    ...
                    if parent.exists():
                        for f in parent.iterdir():
                            if f.name.lower() == abs_path.name.lower():
                                ...
                                break
                    if suggestion:
                        print(f"[CASE MISMATCH] {file_path}: '{ref}' → Did you mean: '{parent}/{suggestion}'?")
                    else:
    ...
    def run(self) -> None:
        ...
        if references:
            print(f"Found {sum(len(refs) for refs in references.values())} potential case sensitivity issues")
    ...
    def dry_run(self) -> None:
        ...
        if references:
            print(f"Would fix {sum(len(refs) for refs in references.values())} case sensitivity issues:")
            for file_path, refs in references.items():
                print(f"\n{file_path}:")
...
def main():
    ...
    if target_file:
        ...
        if target_file in references:
            ...
            if dry_run:
                print(f"Would fix {len(target_refs[target_file])} case sensitivity issues in {target_file}")
                for _, old_ref, new_ref in target_refs[target_file]:
                    print(f"  {old_ref} -> {new_ref}")
            else:
                fixer.fix_references(target_refs)
                print(f"Fixed {len(fixer.fixes_made)} case sensitivity issues in {target_file}")
