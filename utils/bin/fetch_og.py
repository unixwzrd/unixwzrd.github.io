...
def cache_image(image_url, repo_name, web_root):
 ...
 # For remote URLs, always try to cache the image
 ...
 # Fall back to using the remote image URL directly instead of a generic placeholder
 logger.warning("Falling back to remote image URL for %s due to invalid content type", repo_name)
 return image_url
 ...
 logger.warning("Keeping existing image for %s: %s", repo_name, image_path)
 return f'/assets/images/projects/{repo_name}.png'
 # Fall back to remote URL so GitHub OG images can still render
 logger.warning("Falling back to remote image URL for %s due to small content", repo_name)
 return image_url
 ...
 logger.warning("Keeping existing image for %s: %s", repo_name, image_path)
 return f'/assets/images/projects/{repo_name}.png'
 # As a last resort, return the remote URL rather than a generic placeholder
 logger.warning("Falling back to remote image URL for %s due to request error", repo_name)
 return image_url
...
def create_project_entry(data: Optional[Dict], owner: str, name: str, base_dir: Path, repo_config: Dict) -> Dict:
 ...
 # Determine default description based on visibility
 visibility = manual_data.get('visibility', "private")
 default_description = f"Private repository: {name}" if visibility != "public" else f"Public repository: {name}"

 return {
 "name": name,
 "owner": owner,
 "title": manual_data.get('title', format_title(name)),
 "description": manual_data.get('description', default_description),
 "image_url": cached_image,
 "page_url": f"/projects/{name}/",
 # Allow manual override of visibility/repo_url for soon-to-be-public projects
 "visibility": visibility,
 "repo_url": manual_data.get('repo_url', f"https://github.com/{owner}/{name}")
 }

 # For public repos, use GitHub/OpenGraph data, but allow manual overrides
 # OG data has 'image_url' directly, GitHub API data might have it in owner.avatar_url
 image_url = manual_data.get('image_url')
 if not image_url:
 # Try OG data first (from fetch_og_metadata), then GitHub API data
 image_url = data.get('image_url') or (data.get('owner', {}).get('avatar_url') if isinstance(data.get('owner'), dict) else '')
 if not image_url and existing_image_url:
 image_url = existing_image_url
 elif not image_url:
 image_url = '/assets/images/projects/default.png'
 ...
 "owner": owner,
 "title": title,
 "description": manual_data.get('description', data.get('description', '')),
 "image_url": cache_image(image_url, name, base_dir),
 "repo_url": manual_data.get('repo_url', f"https://github.com/{owner}/{name}"),
 "page_url": f"/projects/{name}/",
 "visibility": manual_data.get('visibility', data.get('visibility', 'public'))
 }
...
def main():
 ...
 # Check if manual_data overrides visibility to public
 manual_data = repo.get('manual_data', {})
 is_public_override = manual_data.get('visibility') == 'public'

 og_data = {}
 # Fetch OG data if repo is public (via GitHub API) OR if manually marked as public
 if github_data or is_public_override:
 try:
 og_data = fetch_og_metadata(f"https://github.com/{owner}/{name}") or {}
 if og_data and og_data.get('title'):
 logger.info("Retrieved OG title for %s/%s: %s", owner, name, og_data.get('title'))
 elif og_data:
 logger.warning("OG data retrieved for %s/%s but no title found", owner, name)
 except Exception as e:
 # OG fetch may fail for private repos even if marked public - that's OK
 logger.debug("OG fetch failed for %s/%s (may be private): %s", owner, name, e)
 og_data = {}

 # Merge GitHub and OpenGraph data, preferring OpenGraph for overlapping fields
 # If github_data is None but we have OG data (from public override), use OG data
 merged_data = None
 if github_data:
 merged_data = {**github_data, **og_data}
 elif og_data:
 # Use OG data even if GitHub API says private (when visibility: public override)
 merged_data = og_data
