...
def fetch_url_data(url: str, parse_html: bool = False) -> Optional[Dict]:
    ...
    try:
        ...
        if parse_html:
            ...
            logger.info(
                "  - description: %s", og_desc["content"] if og_desc else "Not found"
            )
            logger.info(
...
def cache_image(image_url, repo_name, web_root):
    ...
    if image_url.startswith("/"):
        ...
        if local_image_path.exists():
            ...
            logger.error(
                "Local image not found for %s: %s", repo_name, local_image_path
            )
            return get_first_existing_fallback()
    ...
    try:
        ...
        if not content_type.startswith("image/"):
            ...
            if image_path.exists():
                logger.warning(
                    "Keeping existing image for %s: %s", repo_name, image_path
                )
        ...
        if len(content) < 1000:  # Minimum size check
            logger.error(
                f"Image content too small for {repo_name}, likely an error response"
            )
            if image_path.exists():
                logger.warning(
                    "Keeping existing image for %s: %s", repo_name, image_path
...
{project_data['description']}"""
            ...
            logger.info(
                f"Project page for {project_data['name']} already exists, skipping..."
            )
...
"""
        ...
        sample_post = (
            posts_dir / f"{current_date}-{project_data['name'].lower()}-introduction.md"
        )
...
"""
        ...
        logger.error(
            f"Failed to generate project files for {project_data['name']}: {e}"
        )
        return False
