...
layout: post
title: "UnicodeFix v2.0.0: The Ghostmark Edition"
date: 2026-08-20
category: UnicodeFix
tags: [release, unicode, text-forensics, provenance, c2pa, watermarking, authorship-signals, markdown, source-code, privacy, python, open-source]
excerpt: "UnicodeFix v2.0.0 introduces evidence-based local auditing for Unicode security, provenance, configured watermark signals, authorship signals, typography, and formatting—with cleanup limited to what it can identify safely."
...
**UnicodeFix v2.0.0 — the Ghostmark Edition** is built around that distinction.
...
[Get UnicodeFix v2.0.0 on GitHub](https://github.com/unixwzrd/UnicodeFix)
...
Instead, v2.0.0 introduces a shared findings model used by human-readable, JSON, and CSV reports. Findings are separated into six categories:
...
The release gate now runs Black, Ruff, pytest, the installed console script, the shell integration suite, and ShellCheck across maintained shell scripts. The final v2.0.0 refresh also aligns source parsing with tree-sitter 0.25 and 0.26 so supported source languages do not silently fall back to a generic lexer.
...
Everything runs locally after installation. UnicodeFix does not upload text to a detector, vendor, or model service, and it does not dereference external provenance manifests during ordinary operation.
