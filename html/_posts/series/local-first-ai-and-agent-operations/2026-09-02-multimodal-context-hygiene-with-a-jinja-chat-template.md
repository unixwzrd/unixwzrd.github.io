...
The first time I saw an image tool result carried forward into a later prompt, the waste was obvious. A long textual payload sat in the conversation history even though the useful work had already happened. Then I found another copy in an assistant tool call that decoded or saved the same bytes. The model was being asked to carry old media work through turns where it was no longer useful.

That kind of history can become expensive quickly. Base64 expands binary data, and the rendered text still has to occupy context and participate in the model's attention and KV-cache work. I am deliberately not attaching a percentage to that statement because the effect depends on the payload, tokenizer, context size, cache behavior, and runtime. I did not need a benchmark to establish the first engineering fact: repeating thousands of characters of old encoded media is more work than omitting them.

The tempting fix was to strip the payload in the model proxy. I had just spent the [previous installment]({{ page.series_previous_url | relative_url }}) establishing that the proxy was a passive diagnostic instrument, though, and letting it rewrite requests would have destroyed the boundary that made its evidence trustworthy. The filtering needed to happen somewhere else. For the Qwen route I was working with, that place was the selected Jinja chat template.

<!--more-->

## The Template Is Part of the Model Interface

An OpenAI-compatible request is not the prompt the model consumes. It is structured input: messages, roles, tool definitions, tool calls, and sometimes native image or video parts. The chat template turns that structure into the model's own grammar, and in this case that grammar belongs to the Qwen 3.5 family. It includes the `<|im_start|>` and `<|im_end|>` turn envelope, Qwen vision placeholders, thinking layout, tool-call serialization, validation rules, and the final assistant generation boundary. Calling the file a generic ChatML template would hide most of the compatibility contract that matters.

I kept the upstream stock template unchanged as the baseline and built the media-history policy as a derivative. That decision gives me a real fallback, not a reconstruction based on memory, and it gives the tests a useful control case: ordinary text and tool history should render identically under the stock and derived templates. The proxy still observes the exchange without changing it, while the selected template performs the intentional transformation when the runtime constructs the prompt. If I want the proxy's rendered diagnostic to match what llama.cpp constructs, both must reference the same template file.
...
The first version of this work kept only the latest image-like payload. That proved the basic idea, but it was too broad in one direction and too narrow in another. A long string containing an image signature is not necessarily an image result, and removing only a payload can leave a broken conversational exchange behind.
...
The 4,096-character threshold is a routing gate. It is not a truncation size, a token estimate, or proof that the content is valid base64. Jinja is not a media parser, and I do not want a template pretending it has validated data it merely recognized by structure and marker. False positives can be just as damaging as missed cleanup, so one synthetic fixture contains a long ordinary diagnostic result with an incidental PNG signature. It stays in the rendered prompt because the surrounding result structure does not identify it as an image result.{% if hands_on_link_ready %} You can exercise the exact greater-than boundary at [4,096 and 4,097 characters in the lab]({{ page.series_companion_url | relative_url }}#step-7-touch-the-4096-character-boundary).{% endif %}
...
The current policy removes every textual image tool result, including the latest one. That may sound aggressive until the distinction between textual history and native structured vision is clear. Textual image results are often enormous, may already be truncated, and cannot be relied upon to reconstruct a usable image for the model. Keeping the most recent damaged string does not make it useful. Native structured image parts are different because the model engine handles their data separately while the template emits Qwen's vision placeholder, so those parts remain eligible.
...
A tool result has conversational context. An assistant requested the tool, the tool returned a result, and later turns may refer to that exchange. If I remove only the result, I can leave an orphaned assistant call in the prompt, which leaves the model looking at a request for work with no matching response. The template therefore removes media-producing assistant calls and their associated tool results as complete exchanges. It also removes assistant calls that carry recognizable media bytes or explicitly decode base64, plus the following tool results belonging to those calls. This catches the second copy that originally drew my attention: the image had already appeared as a tool result, then an assistant-side operation copied or decoded the same bytes again.
...
I built the publication companion around seven invented fixtures rather than a sanitized screenshot from my own environment. The fixtures generate obvious canary strings locally and never contain a real prompt, conversation, model response, or media artifact.{% if hands_on_link_ready %} The Hands-On companion lets you [compare all seven fixtures without dumping their long payloads]({{ page.series_companion_url | relative_url }}#step-4-compare-all-seven-fixtures-without-dumping-the-payloads).{% endif %}
...
Replacing a model's bundled chat template is not a cosmetic configuration change. A missing delimiter, changed tool schema, damaged reasoning boundary, or misplaced generation marker can make a healthy model behave as though the whole serving stack is broken.
...
Rollback is the stock template kept beside the derivative. Restoring its path and repeating the same canaries is faster and safer than editing the derived template under pressure, which is exactly when a hurried template repair is least attractive. The companion README includes the profile shape, restart sequence, checksums, acceptance gates, and stock rollback procedure.
...
The technical work that remains is narrower. I want an explicit final-artifact generation acceptance pass against the selected model revision, plus repeated checks for ordinary tools, native vision, reasoning boundaries, and termination. More general tool-call reduction remains deferred because it can change semantics in ways that media-history cleanup does not.
